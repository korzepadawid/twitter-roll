...
func New(c *config.Config, l *zap.Logger) Twitter {
	return &Client{
		logger:      l,
...
func (tC *Client) newAPIRequest(ctx context.Context, r *twitterRequest) (*http.Request, error) {
...
func (tC *Client) getRulesIDs() ([]string, error) {
...
func (tC *Client) deleteStreamRules(ids []string) error {
	...
	err := json.NewEncoder(reqBodyBuff).Encode(body)
	if err != nil {
		return err
	}
	req, err := tC.newAPIRequest(ctx, &twitterRequest{
...
// CleanStreamRules Removes already existing stream rules for your Twitter account.
// Results won't be affected by unwanted rules.
func (tC *Client) CleanStreamRules() error {
...
// CreateStreamRules creates given stream rules, otherwise returns an error
func (tC *Client) CreateStreamRules(r []StreamRule) error {
